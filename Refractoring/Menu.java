...
public class Menu extends JFrame{
	...
	{	
		...
		lodgementButton.addActionListener(new ActionListener(  ) {
			...
			// String date = new SimpleDateFormat("yyyyMMdd_HHmmss").format(Calendar.getInstance().getTime());
			 ...
			 recordTransaction(acc, "Lodgement", balance);
		...
		withdrawButton.addActionListener(new ActionListener(  ) {
			...
			//		String date = new SimpleDateFormat("yyyyMMdd_HHmmss").format(Calendar.getInstance().getTime());
				 recordTransaction(acc, "Withdraw", withdraw);
	...
	private boolean verifyPinIfRequired(CustomerAccount account) 
	{
	    ...
	    int attempts = 3;
	    int correctPin = ((CustomerCurrentAccount) account).getAtm().getPin();

	    while (attempts > 0)
	    {
	        String input = JOptionPane.showInputDialog(f, "enter 4 digit pin ");
	        int enteredPin = Integer.parseInt(input);

	        if (enteredPin == correctPin) 
	        {
	            JOptionPane.showMessageDialog(f, "pin  successful", "Pin", JOptionPane.INFORMATION_MESSAGE);
	            return true;
	        } else
	        {
	            attempts--;
	            JOptionPane.showMessageDialog(f,
	                    "Incorrect pin. " + attempts + " attempts remaining.","Pin",
	                    JOptionPane.INFORMATION_MESSAGE);
	        }
	    }

	    JOptionPane.showMessageDialog(f,"Pin  incorrect ATM card locked.", "Pin", JOptionPane.INFORMATION_MESSAGE);

	    ((CustomerCurrentAccount) account).getAtm().setValid(false);
	    return false;
	}
	
	private void recordTransaction(CustomerAccount account, String type, double amount)
	{
	    Date date = new Date();
